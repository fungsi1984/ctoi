...
public:
    vector<int> maximumSumQueries(vector<int> &nums1, vector<int> &nums2,
        ...
        const vector<pair<int, int>> pairs = getPairs(nums1, nums2);
        vector<int> ans(queries.size());
        vector<pair<int, int>> stack; // [(y, x + y)]

        int pairsIndex = 0;
        for (const auto &[queryIndex, minX, minY] :
             getIndexedQueries(queries)) {
            while (pairsIndex < pairs.size() && pairs[pairsIndex].first >= minX) {
            ...
            const auto it =
                ranges::lower_bound(stack, pair<int, int>{minY, INT_MIN});
...
private:
    vector<pair<int, int>> getPairs(const vector<int> &nums1, const vector<int> &nums2) {
        vector<pair<int, int>> pairs;
        for (int i = 0; i < nums1.size(); ++i)
            pairs.push_back({nums1[i], nums2[i]});
        ranges::sort(pairs, ranges::greater{},
                     [](const pair<int, int> &pair) { return pair.first; });
        return pairs;
    }
...
int main() {
    ...
    Solution solution;
    vector<int> result = solution.maximumSumQueries(nums1, nums2, queries);
